# ======================================================================================
# SOLVER MULTIFASE CON OVERFLOW EN CRECIMIENTO  (celda 46b)
# ======================================================================================
# Reemplaza al solver "balanced" (celda 46) como solver preferido del forward.
# Diferencias clave vs celda 46 (balanced):
#   - 5 fases (lag / growth / growth_N_limited / stationary / decay)
#   - En growth: mu >= ALPHA_GROWTH * mu*  (deja 5% para overflow)
#   - Overflow obj: ATPM + isoamylol + isoamyl_acetate + ethyl_acetate (pesos PSO-tunable)
#   - Turnover SOLO en stationary/decay
#   - Ester soft constraints: heredadas del model global (celda 65 las instala)
#
# USO: Ejecutar esta celda DESPUES de celdas 43-46 (necesita todos los globals de ahi).
#      Luego forward_simulate (celda 65) la detecta automaticamente.
# ======================================================================================
...
print("\n" + "=" * 100)
print("Solver multifase con overflow en crecimiento (celda 46b)")
print("=" * 100)

# ── Aliases locales (consistentes con celda 45) ──
...
# ── Parametros multifase (PSO los sobreescribe via globals en apply_theta) ──
...
W_ETHYL_ACETATE_GROWTH      = float(globals().get("W_ETHYL_ACETATE_GROWTH", 0.2))
...
print(f"ALPHA_GROWTH={ALPHA_GROWTH}  W_ATPM={W_ATPM_GROWTH}  "
      f"W_ISO={W_ISOAMYLOL_GROWTH}  W_IAA={W_ISOAMYL_ACETATE_GROWTH}  W_EA={W_ETHYL_ACETATE_GROWTH}")
print(f"LAG_END_H={LAG_END_H}  N_LIM_THR={N_LIMITED_THRESHOLD}  DECAY_H={DECAY_START_H}")

# Collocation constants (Radau IIA, 3-point) — nombres locales para evitar colision
_COLMAT_MP = np.array([
    [0.19681547722366, -0.06553542585020,  0.02377097434822],
    [0.39442431473909,  0.29207341166523, -0.04154875212600],
    [0.37640306270047,  0.51248582618842,  0.11111111111111],
])
_COLMAT_MP_INV = np.linalg.inv(_COLMAT_MP)
_RADAU_MP = np.array([0.15505102572168, 0.64494897427832, 1.0])
...
def _infer_phase(cN_total, t_h):
    """Infiere la fase biologica segun N total disponible y tiempo."""
    _lag_end = float(globals().get("LAG_END_H", LAG_END_H))
    _n_lim   = float(globals().get("N_LIMITED_THRESHOLD", N_LIMITED_THRESHOLD))
    _decay_h = float(globals().get("DECAY_START_H", DECAY_START_H))

    if t_h < _lag_end:
        return "lag"
    if cN_total <= N_TOTAL_DEPLETION_THRESHOLD:
        return "decay" if t_h >= _decay_h else "stationary"
    if cN_total <= _n_lim:
...
# ══════════════════════════════════════════════════════════════════════════════════════
# LP SOLVER MULTIFASE  (punto de colocacion)
# ══════════════════════════════════════════════════════════════════════════════════════

def _fba_at_state_colloc_multiphase(yk, aerobic_mode, t_h=0.0):
    """
    Resuelve FBA en un punto de colocacion con logica multifase.

    GROWTH/GROWTH_NLIM:
      Step A: max mu -> obtiene mu*
      Step B: epsilon-constraint  mu >= alpha * mu*
      Step C: max overflow = w_atpm*ATPM + w_iso*isoamylol + w_iaa*isoamyl_acetate
              + w_ea*ethyl_acetate  (pFBA para parsimonia)

    STATIONARY/DECAY:
      max protein (con floor), luego max ATPM. Turnover activo.

    LAG:
      max ATPM, sin crecimiento.

    Retorna:
      (fluxes_dict, mode_str, phase_str) si exito
      (None, "FAIL"/"NO_OPT", phase_str) si fallo
    """
    ...
    with model as mtmp:
        ...
        # ── GROWTH / GROWTH_NLIM ──
            ...
            # Re-read globals for PSO tunability
            _alpha = float(globals().get("ALPHA_GROWTH_NLIM", ALPHA_GROWTH_NLIM)) if is_nlim \
                     else float(globals().get("ALPHA_GROWTH", ALPHA_GROWTH))
            ...
            # Step B: epsilon-constraint (near-optimal growth)
            rb[OBJ_ID].lower_bound = max(rb[OBJ_ID].lower_bound,
                                          float(_alpha) * mu_star)

            # Step C: max overflow (ATPM + isoamylol + isoamyl_acetate + ethyl_acetate)
            _w_atpm = float(globals().get("W_ATPM_GROWTH", W_ATPM_GROWTH))
            _w_iso  = float(globals().get("W_ISOAMYLOL_GROWTH", W_ISOAMYLOL_GROWTH))
            _w_iaa  = float(globals().get("W_ISOAMYL_ACETATE_GROWTH", W_ISOAMYL_ACETATE_GROWTH))
            _w_ea   = float(globals().get("W_ETHYL_ACETATE_GROWTH", W_ETHYL_ACETATE_GROWTH))
            if is_nlim:
                _w_iso *= 1.5
                _w_iaa *= 1.5
                _w_ea  *= 1.5
            ...
            if _w_ea > 0 and ETHYL_ACETATE_RID in rb:
                mix_obj[mtmp.reactions.get_by_id(ETHYL_ACETATE_RID)] = float(_w_ea)
...
def _compute_rhs_colloc_multiphase(yk, fluxes, mode, phase):
    """Calcula dC/dt con turnover activo SOLO en stationary/decay."""
    cX    = max(0.0, yk[IDX_X])
...
# ══════════════════════════════════════════════════════════════════════════════════════
# dFBA SOA MULTIFASE — Radau IIA collocation
# ══════════════════════════════════════════════════════════════════════════════════════
...
def _run_dfba_colloc_multiphase(
    *,
    ...
    """
    Integra dFBA con Radau IIA (SOA) usando el solver multifase.
    Retorna dict con states_df, flux_df, C, CDOT, C_endpoints, etc.
    Compatible con forward_simulate (misma interfaz que _run_dfba_colloc_balanced).
    """
    assert ncp == 3, "Solo soporta ncp=3 (Radau IIA de 3 puntos)"
    h = t_end / nfe
    hm = np.full(nfe, h)
    nc = n_state

    t_fe_start = np.concatenate([[0.0], np.cumsum(hm[:-1])])
    t_loc = np.array([[t_fe_start[i] + _RADAU_MP[j] * h
    ...
    for i in range(nfe):
        ...
        for _it in range(n_iter):
            ...
            # Collocation update
            C_new = c_prev[:, None] + h * (CDOT[:, i, :] @ _COLMAT_MP.T)
            C[:, i, :] = np.maximum(C_new, 0.0)

        c_prev = C[:, i, ncp - 1].copy()

        # N pulses at end of FE
        _t_start_fe = h * i
        _t_end_fe   = h * (i + 1)
    ...
    for i in range(nfe):
        ...
        for s in range(nc):
            ...
            cdot_coll = _COLMAT_MP_INV @ rhs_j
    ...
    print(f"\n[{scenario_name}] Solver multifase completado.")
...
print("Solver multifase registrado: _run_dfba_colloc_multiphase, _fba_at_state_colloc_multiphase")
